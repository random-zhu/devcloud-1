  ...
  <div class="container-wrapper">
    ...
    >
      ...
      <el-table-column label="访问标签" min-width="110px">
        <template slot-scope="{ row }">
          <div v-if="row.entry.labels">
            <div
              v-for="(value, name) in row.entry.labels"
              :key="name"
              class="github-badge"
            >
              <span class="badge-subject">{{ name }}</span
              ><span class="badge-value bg-blue">{{ value }}</span>
            </div>
          </div>
          <span v-else> - </span>
        </template>
      ...
      <el-table-column label="访问方法" min-width="60px">
        <template slot-scope="{ row }">
          <el-tag effect="plain" :style="methodColer(row.entry.method)">{{
            row.entry.method
          }}</el-tag>
        </template>
      </el-table-column>
    ...
    <pagination
      :total="total"
      :page.sync="listEndpointQuery.page_number"
      :limit.sync="listEndpointQuery.page_size"
      :page-sizes="[10, 20, 50, 100, 200, 500]"
      @pagination="getServiceEndpoint"
    />
  </div>
</template>
...
import { QUERY_ENDPOINT } from "@/api/keyauth/endpoint";
  ...
  methods: {
    ...
    async getServiceEndpoint() {
      ...
      try {
        ...
        let resp = await QUERY_ENDPOINT(this.listEndpointQuery);
