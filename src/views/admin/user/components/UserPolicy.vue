  ...
  <div class="container-wrapper">
    ...
    <div class="table-operate-zone">
      ...
      <div class="table-operate">
        <el-button type="primary" size="mini" @click="handleCreatePolicy()"
          >添加策略</el-button
        >
      </div>
    </div>
    ...
    <div>
      ...
      >
        ...
        >
          <template slot-scope="{ row }">
            <router-link
              :to="'/permission/role/' + row.role_id"
              class="link-type"
            >
      ...
      <pagination
        v-show="total > 0"
        :total="total"
        :page.sync="listPolicyQuery.page_number"
        :limit.sync="listPolicyQuery.page_size"
        @pagination="getNamespacePolicy"
      />
    </div>
...
<script>
import { QUERY_POLICY } from "@/api/keyauth/policy";
import Pagination from "@/components/Pagination";
import CreatePolicyDrawer from "@/views/admin/policy/components/CreatePolicyDrawer";
import Tips from "@/components/Tips";

const tips = ["用户在对应空间下所属角色"];
...
export default {
  ...
  methods: {
    getNamespacePolicy() {
      ...
      QUERY_POLICY(this.listPolicyQuery)
        .then((resp) => {
