    ...
    <div class="drawer-content">
      <el-form ref="userForm" :model="form" :rules="rules">
        ...
        >
          <choice-department
            v-if="!departmentId"
            :department.sync="form.department_id"
          />
        ...
        >
          ...
          <div class="fr" style="width: 20%; display: inline-flex">
            <el-button type="text" @click="generateRandomPass">
              <svg-icon
                style="font-size: 18px; color: #13c2c2; margin-left: 12px"
                icon-class="random"
            /></el-button>
            <el-button
        ...
        <el-form-item
          label="姓名"
          :label-width="formLabelWidth"
          prop="profile.real_name"
        >
          <el-input
            v-model="form.profile.real_name"
            maxlength="40"
            show-word-limit
          />
          <div class="input-tips">
        ...
        <el-form-item
          label="昵称"
          :label-width="formLabelWidth"
          prop="profile.nick_name"
        >
          <el-input
            v-model="form.profile.nick_name"
            maxlength="40"
            show-word-limit
          />
          <div class="input-tips">
        ...
        >
          ...
          <el-radio v-model="form.profile.gender" label="UNKNOWN"
            >保密</el-radio
          >
        </el-form-item>
      ...
      <div class="drawer-footer">
        ...
        <el-button
          ...
          @click="submit"
          >{{ createAccountLoading ? "提交中 ..." : "确 定" }}</el-button
        >
      </div>
    </div>
  </el-drawer>
...
<script>
import { CREATE_ACCOUNT, GEN_RANDOM_PASSWORD } from "@/api/keyauth/user";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        "profile.phone": [
          { required: true, message: "请输入用户电话号码", trigger: "change" },
        ],
        "profile.email": [
  ...
  methods: {
    ...
    async generateRandomPass() {
      var resp = await GEN_RANDOM_PASSWORD({});
    ...
    submit() {
      this.$refs["userForm"].validate(async (valid) => {
        if (valid) {
          ...
          try {
            let resp = await CREATE_ACCOUNT(this.form);
