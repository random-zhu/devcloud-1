    ...
    <div class="drawer-content">
      <el-form ref="policyForm" :model="form" :rules="rules">
        ...
        >
          <el-select
            ...
            style="width: 100%"
            placeholder="请选择授权空间"
            :loading="queryNamespaceLoading"
            @visible-change="showNamespaceList"
          >
            <el-option
        ...
        >
          ...
          >
            ...
            >
              ...
              >
              <span v-else style="float: right; color: #8492a6; font-size: 12px"
                >无部门</span
              >
            </el-option>
          </el-select>
        ...
        <el-form-item label="范围" :label-width="formLabelWidth">
          <choice-tag
            :tag-key.sync="scope_key"
            :tag-values.sync="scope_values"
            @change="handleTagChanged"
          />
        </el-form-item>
      ...
      <div class="drawer-footer">
        ...
        <el-button
          ...
          @click="submit"
          >{{ createPolicyLoading ? "提交中 ..." : "确 定" }}</el-button
        >
      </div>
    </div>
  </el-drawer>
...
import { QUERY_NAMESPACE } from "@/api/keyauth/namespace";
import { QUERY_SUB_ACCOUNT } from "@/api/keyauth/user";
import { CREATE_POLICY } from "@/api/keyauth/policy";
import ChoiceTag from "../../tag/components/ChoiceTag.vue";
...
export default {
  ...
  methods: {
    ...
    searchSubAccount(keywords) {
      ...
      QUERY_SUB_ACCOUNT(this.subAccountQuery)
        .then((response) => {
    ...
    getNamespaceList() {
      ...
      // 获取用户列表
      QUERY_NAMESPACE(this.namespaceListQuery)
        .then((response) => {
