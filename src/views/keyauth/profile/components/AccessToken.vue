      ...
      >
        ...
        >
          <template slot-scope="{ row }">
            ...
            <el-button
              v-clipboard:copy="row.access_token"
        ...
        <el-table-column label="操作" align="center" width="230">
          <template slot-scope="{ row, $index }">
            ...
            <el-button
              :loading="deleteLoading === row.access_token"
              size="mini"
              style="color: #f56c6c"
              type="text"
              @click="handleDeleteToken(row, $index)"
            >
              删除
            </el-button>
          </template>
      ...
      >
        ...
        >
          ...
          <el-form-item label="过期时间" prop="description">
            ...
            />
            <el-checkbox
              v-model="createTokenForm.not_expired"
...
import { QUERY_TOKEN, DELETE_TOKEN, LOGIN } from "@/api/keyauth/token";
...
export default {
  ...
  methods: {
    ...
    async queryToken() {
      ...
      try {
        var resp = await QUERY_TOKEN({ grant_type: "access" });
    ...
    async handleDeleteToken(tk) {
      ...
      try {
        await DELETE_TOKEN({ access_token: [tk.access_token] });
        this.queryToken();
      } finally {
        this.deleteLoading = "";
      }
